...
let autoSkipTimer;
const currentUser = { authenticated: true, active: true, role: 'user' };
let playerSource = '流光影视';
let playerEpisodeCount = 12;
...
function renderPlayerEpisodes() {
  $('#playerEpisodeGrid').innerHTML = Array.from({ length: playerEpisodeCount }, (_, index) => {
    const number = index + 1;
    return `<button class="${number === episode ? 'active' : ''}" data-player-episode="${number}">第 ${number} 集</button>`;
  }).join('');
}
...
function openQuickSearch() {
  ...
  $('.search-icon').setAttribute('aria-expanded', 'true');
  setTimeout(() => $('#quickQuery').focus(), 80);
}

function closeQuickSearch() {
  if ($('#quickSearch').open) $('#quickSearch').close();
  $('.search-icon').setAttribute('aria-expanded', 'false');
}
...
function reset() {
  ...
  playerSource = '流光影视';
  playerEpisodeCount = 12;
  $$('#playerSourceTabs button').forEach((node, index) => node.classList.toggle('active', index === 0));
  $('#episodeSourceStatus').textContent = '流光影视 · 线路 1 · 12 集';
  renderPlayerEpisodes();
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'toggle-search') $('#quickSearch').open ? closeQuickSearch() : openQuickSearch();
  if (action === 'close-search') closeQuickSearch();
  ...
  if (action === 'mark-start') {
    if (!currentUser.authenticated || !currentUser.active) { toast('登录且账户有效后才能标记广告'); return; }
    markStarted = true; $('#markStart').classList.add('hidden'); $('#markEnd').classList.remove('hidden'); $('#markHint').textContent = '已记录开始 18:42，请在广告结束时点击结束'; toast('已记录广告开始时间 18:42');
  }
  ...
  if (control.dataset.playerSource) {
    playerSource = control.dataset.playerSource;
    playerEpisodeCount = Number(control.dataset.count);
    $$('#playerSourceTabs button').forEach((node) => node.classList.toggle('active', node === control));
    $('#episodeSourceStatus').textContent = `${playerSource} · ${control.dataset.line} · ${playerEpisodeCount} 集`;
    renderPlayerEpisodes();
    toast(`已加载${playerSource}的剧集列表`);
  }
  if (control.dataset.playerEpisode) {
    episode = Number(control.dataset.playerEpisode);
    resetPlayer();
    $('#playbackReady').classList.add('hidden');
    $('#sessionLoading').classList.remove('hidden');
    $('#sessionLoading b').textContent = `正在连接${playerSource}`;
    $('#sessionLoading span').textContent = `第 ${episode} 集 · 正在创建安全播放会话`;
    renderPlayerEpisodes();
    setTimeout(() => {
      $('#sessionLoading').classList.add('hidden'); $('#playbackReady').classList.remove('hidden');
      $('#playingMeta').textContent = `第 ${episode} 集 · ${playerSource}`;
      $('#playbackReady span').textContent = `正在播放 · 第 ${episode} 集`;
      toast(`已切换到${playerSource} · 第 ${episode} 集`);
    }, 850);
  }
  if (control.dataset.searchValue) {
    $('#quickQuery').value = control.dataset.searchValue;
    $('#quickQuery').focus();
  }
});
...
$('#quickSearch').addEventListener('close', () => $('.search-icon').setAttribute('aria-expanded', 'false'));
$('#quickSearch').addEventListener('click', (event) => { if (event.target === $('#quickSearch')) closeQuickSearch(); });
$('#autoSkip').addEventListener('change', () => toast($('#autoSkip').checked ? '已开启可信广告自动跳过' : '已关闭广告自动跳过'));
document.addEventListener('keydown', (event) => {
  if (event.key === '/' && !/input|textarea|select/i.test(event.target.tagName)) { event.preventDefault(); openQuickSearch(); }
});
updateSelection();
