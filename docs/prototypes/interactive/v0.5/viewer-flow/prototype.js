...
function openQuickSearch() {
  ...
  setTimeout(() => $('#quickQuery').focus(), 220);
}
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'toggle-search') $('#quickSearch').classList.contains('open') ? closeQuickSearch() : openQuickSearch();
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'Escape' && $('#quickSearch').classList.contains('open')) closeQuickSearch();
});
updateSelection();
